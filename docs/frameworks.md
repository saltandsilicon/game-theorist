...
- Salary negotiation when both sides have leverage

**What to do**: Change the payoff structure (make defection costly via contract), create
repeated-game dynamics (build a relationship so reputation matters), or find a commitment device
that makes cooperation credible.
...
- **Winner's Curse**: In common-value auctions (where the prize is worth the same to all bidders),
  the winner tends to have the most optimistic estimate and systematically overpays. Optimal bidding
  requires shading your bid downward relative to your estimate.
...
quality also send this signal at a similar cost? If yes, the signal is noise.
...
- Long-term vendor relationships (both sides cooperate because they'll interact again)
